...
has read, which is exactly what generic means. No amount of prompt engineering downstream fixes an
...
Abstractions are compressed memories. The compression is where the specificity went, and asking for
the instance is how you get it back. Expect to do this three or four times in a single interview; it
is not a sign the person is being evasive, it is just how people talk.
...
If a signal is genuinely missing after you have asked for it twice, say so and stop. A hole in the
brief is a finding. It tells the drafting step to write around the gap rather than confabulate across
it.
...
This matters more than it sounds. Paraphrasing launders their voice back out before drafting even
starts, and you end up with a brief written in the same neutral register that made the AI draft
generic in the first place. Quote them. Keep the odd phrasing. Keep the aside that seemed
tangential.
...
HOLES
  <what was asked for and not obtained; the drafting step must not fill these in>
```

**Never fabricate to fill a gap.** Not a plausible number, not a representative example, not a
composite story. The entire purpose of this skill is that the drafting step has real material; a
single invented detail in the brief defeats it and, worse, becomes indistinguishable from the real
ones the moment the brief is handed off.

**The brief is ephemeral.** It exists to be handed to the drafting step and then discarded. It is not
a knowledge store, a profile, or a record. Treat it as scaffolding.
...
That is a complete brief: story, point of view, number, and a line worth keeping. Note that "a
dashboard is not a decision" came out of the writer, not the interviewer. It would not have appeared
in a draft written from "write a post about not buying tools too early."
