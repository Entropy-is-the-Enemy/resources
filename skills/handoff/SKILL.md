...
Before writing anything, look for an existing handoff doc so you update it rather than starting a new one from scratch. Check wherever your project notes and outputs live - the folder you save deliverables to, and anywhere you keep persistent project reference. A quick search for filenames containing "handoff" is usually enough.
...
Save as `[project-name]-handoff.md`. Put it wherever your project notes live so it persists for the next session - the folder you keep persistent reference in, or failing that, your usual outputs folder. The point is that the next session can find and attach it.
