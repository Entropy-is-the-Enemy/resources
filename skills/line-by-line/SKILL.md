...
Capture the answer as a short arrival state, in parts, so every earlier unit has a fixed target to aim at. Use this template model (from the worked example: a beginner tutorial deck):
...
See `references/worked-example.md` for a full walkthrough: building a 60-minute beginner tutorial deck end-first, one walkable slide at a time, including the mid-build corrections the method made cheap.
