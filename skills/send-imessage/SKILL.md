...
the bubble is up. That needs a shape, not just a tool call.
...
what happened before reporting or retrying. iMessage is the concrete instance.

The transport is your own Mac, reached over a computer-use bridge as `<your-mac-device-id>`. The
Messages UI is the primary path, with a narrow scripted fallback behind it.
...
5. No modal or permission dialog holding focus. A dialog with focus eats the keystrokes silently.
6. Correct display, if more than one is attached. Automation can drive a screen Messages is not on.
...
you expect, click into the compose field, type the body, then **read the composed field back and diff
it character for character against the wording approved at the gate.** System text substitution
silently rewrites quotes, dashes, and expanded shortcuts as you type. If anything differs, repair the
field and re-read it before pressing Return. The UI is preferred because it shows the recipient chip,
the thread history, and whether this sent as iMessage or fell back to SMS.

**Path B: `osascript`.** A one-line AppleScript handing the text to Messages directly. Two refusals
come before any use of it:

- **Refuse Path B on a quoting-fragile body.** If the text contains quote or apostrophe characters, or
  anything else the invocation would have to escape, the inline quoting cannot carry it through
  intact and the message goes out garbled, irreversibly. Any such message goes through
  Path A instead. Ordinary sentences trip this constantly.
- **Refuse Path B without a literal handle.** It does no name resolution: give it a friendly name and
  it will fail or, worse, guess.

It also gives no blue-versus-green signal and no visual thread confirmation, which is why it is the
fallback. Verify Path B sends in the UI. **Opening a brand-new conversation is Path B's most
failure-prone case**, so on a verified non-delivery there, retry on Path A rather than running Path B
again.
...
absent: it failed cleanly and one retry is safe, on Path A if the failed attempt was Path B into a new
conversation. No readable screenshot: report uncertainty and stop.
...
State check:
  ...
  reading Alex Rivera, compose empty, no modal, one display.
  All six ok.
...
Those transports have their own confirmation requirements.

iMessage and AppleScript are trademarks of Apple Inc. This skill is not affiliated with, endorsed by,
or sponsored by Apple.

The four steps generalize to any irreversible outbound action: a resolved target echoed back, an
