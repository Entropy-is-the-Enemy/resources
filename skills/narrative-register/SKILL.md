...
description: >
  ...
  specs, figures, defined terms, dosages, tolerances, warranty or contract language, safety-critical
  instructions, disclosures, or any passage where a reader will act on an exact value. Do NOT apply it
  to a whole document by default or fold it into your standing voice.
---
...
with an initiative, verbs that do physical work, sentence lengths that swing hard, a turn that
reverses what the last sentence set up, a flat funny line delivered without a wink.
...
Classify every block as APPLY or FREEZE and print the map before rewriting a sentence. Short jobs are
where a warranty clause gets swept in.
...
absent from the source. Invented interiority is the most seductive failure here: it reads exactly like
good writing. If a scene wants a detail the source lacks, report the gap.
...
**Once is a move, twice is a tic.** A device at double budget reads as impression, not craft. Before
shipping, count each device by name and report the counts. Over budget, cut the weakest instance, not
the last one.

**Pool budgets when layers stack.** Running this alongside your own house-voice layer, or any layer
with signature moves, means shared budgets, not additive ones. Two layers each spending two reversal
beats gives a document four, which reads like parody. Split the passages first, one layer taking
...
Fragments are a register favorite and the first thing a house style forbids. When a rule blocks a
device, note the substitution rather than dropping the rule.

**Keep four eval fixtures and rerun them after any rule change**: a tolerance clause mid-paragraph
(survives verbatim), a passage seeded with five reversal beats (flagged over budget), a decision with
no stated reason (no motive), a bare table (declined).
...
SCOPE MAP
  ...
  FREEZE: <blocks, reason: spec | number | defined term | legal | safety | quote>
...
FIDELITY
  <what the scene wanted and the source did not supply, never invented>
```
...
what a document should say. It never runs by default and never becomes your standing voice, because a
register applied everywhere stops being a layer and becomes your voice by default. Applied to a spec, a
disclosure, or a dosage, it is not a style choice. It is a defect.
